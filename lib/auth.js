...
const { SESSION_DAYS, getRawSessionToken } = require("./session-cookie");
...
async function getCustomerByToken(rawToken) {
  if (!rawToken || !isDbConfigured()) return null;

  const sessions = await dbRequest("GET", "customer_sessions", {
    query: `token=eq.${encodeURIComponent(rawToken)}&select=customer_id,expires_at&limit=1`,
  });
...
async function getCustomerFromRequest(req) {
  ...
  return getCustomerByToken(rawToken);
}

async function requireCustomer(req, res, sendJson) {
