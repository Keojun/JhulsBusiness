...
const { isAdminSessionValid } = require("./admin-session");
...
function checkAdmin(req, body) {
  ...
  if (password && password === getAdminPassword()) return true;
  // Admin cookie alone is not enough — customer fetch also sends cookies.
  // Admin panel requests must include X-RBXDISC-Admin (see adminFetch in storage.js).
  if (req.headers["x-rbxdisc-admin"] !== "1") return false;
...
function setCors(res) {
  ...
  res.setHeader(
    ...
    "Content-Type, X-Admin-Password, X-RBXDISC-Admin, Authorization"
  );
}
